...
def transform_data():
    ...
    for col in list(df_aapl.columns)[0:4]:
        # We ignore 'Volume' column
        df_aapl = df_aapl.drop(df_aapl[df_aapl[col].values > 900].index)
        # Values above 900 are dropped
        df_aapl = df_aapl.drop(df_aapl[df_aapl[col].values < 0.001].index)
        # Values below 0.001 are dropped
